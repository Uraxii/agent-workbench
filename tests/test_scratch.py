...
def _stub_compose(
    ...
    needed: `port` and `podman image inspect` get canned stdout, anything
    else (build/up/down/the wrapped command) just succeeds.
    """
    ...
    def fake_run(cmd: list[str], *args: object, **kwargs: object):
        ...
        if "inspect" in cmd:
            return SimpleNamespace(
                returncode=0, stdout="deadbeef 2026-01-01T00:00:00Z\n",
            )
