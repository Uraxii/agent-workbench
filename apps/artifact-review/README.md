...
`artifact-serve` compose service. The image contains the Django backend, a
...
podman run --rm -p 127.0.0.1:9099:9099 \
  -e ARTIFACT_SERVE_HOST=0.0.0.0 \
  -e ARTIFACT_SERVE_PORT=9099 \
...
- `ARTIFACT_SERVE_HOST`: host setting read by Django. Compose sets `0.0.0.0`.
- `ARTIFACT_SERVE_PORT`: gunicorn and healthcheck port. Defaults to `9099`.
- `ARTIFACT_SERVE_ALLOWED_HOSTS`: comma-separated Django allowed hosts.
- `ARTIFACT_SERVE_STAGE_ROOT`: published artifact root. Defaults to `/tmp/artifacts`.
- `ARTIFACT_SERVE_FEEDBACK_ROOT`: sqlite feedback and upload root. Defaults to `~/.local/share/artifacts`.
- `ARTIFACT_SERVE_SPA_ROOT`: built SPA root. Defaults to `backend/spa` inside the app.
- `ARTIFACT_SERVE_ASSETS_ROOT`: backend asset root.
- `ARTIFACT_SERVE_PUBLISH_ENABLED`: set `0` to disable HTTP publishing.
