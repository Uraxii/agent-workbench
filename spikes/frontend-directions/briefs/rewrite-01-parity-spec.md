...
- `apps/artifact-review/`  -> THE server. Read it fully. This is the primary source.
- `.claude/skills/agent-workbench/modes/artifact.md`  -> the consumer contract (publish + feedback CLI).
...
2. **Artifact resolution + staging.** How `artifact_id` maps to a staged path, the `/tmp/artifacts/<project>/<subdir>` layout, HTTP publish tar extraction, and unsafe-member rejection.
3. **Database schema.** Every table (artifact_index, comment, thread, and any others), columns, types, indexes, and the DB file location (`~/.local/share/artifacts/feedback.db`). How threads/comments/annotations/resolved-state persist. The bd-board mirror behavior (what mirrors, when, to which board).
...
5. **agent-workbench artifact contract.** The exact `publish` inputs/outputs and the `feedback --artifact <id>` JSON shape the CLI depends on. The backend MUST keep these working (publish -> stable viewer URL; feedback returns JSON). Quote the expected shapes.
6. **Security + hardening bar (non-negotiable, hard-won).** Container: read-only rootfs, `cap_drop: ALL`, `no-new-privileges`, base image pinned by digest, healthcheck, mounts NARROWED to only the artifact dirs (`/tmp/artifacts` + `~/.local/share/artifacts`, NO `$HOME`-wide mount), bind `127.0.0.1:9099`. The host `tailscale serve --https=443` in front, and the live-service footgun: never add a path that tears down host tailscale. The publish stored-XSS hole (agent-workbench-wxh): same-origin active content (user HTML/SVG/JS) on the no-auth UI; list the three acceptable mitigations (sandbox CSP disabling script on the raw artifact route, OR separate origin/port for raw artifacts, OR restrict to non-active types). The SSRF pattern for any server-side fetch (reject loopback/private/link-local/metadata 169.254.169.254/CGNAT 100.64.0.0/10 on initial URL and every redirect hop; reference `scripts/kb-clip.py` closed fix `h5u`). Trust model: no-auth on trusted tailnet.
