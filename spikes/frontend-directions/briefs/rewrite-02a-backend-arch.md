...
2. **API contract.** One table mapping EVERY parity endpoint to its .NET handler (method, route, request DTO, response DTO, status codes). Default: match the existing contract exactly so the `artifact-serve` skill keeps working unchanged. Flag any endpoint you propose to improve and state the lockstep skill-doc change it forces.
3. **Data layer.** Read/write the SAME existing sqlite DB (`~/.local/share/claude-artifacts/feedback.db`) and the SAME staging dirs, so the existing push CLI and feedback CLI keep working with zero change. Document the schema mapping, connection handling, and the bd-board mirror behavior (keep it, or re-decide with justification).
...
7. **Container hardening plan.** Slim, digest-pinned .NET base image; read-only rootfs; `cap_drop: ALL`; `no-new-privileges`; healthcheck; bind `127.0.0.1:9099`; mounts NARROWED to only `/tmp/claude-artifacts` + `~/.local/share/claude-artifacts` (NO `$HOME`-wide mount). Host `tailscale serve --https=443` stays in front; NEVER add a path that tears down host tailscale.
