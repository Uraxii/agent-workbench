...
2. **API contract.** One table mapping EVERY parity endpoint to its Django URL + view (method, route, request shape, response shape, status codes). Default: match the existing contract EXACTLY so the `artifact-svc` skill keeps working unchanged. Flag any endpoint you propose to add/improve (e.g. the Ledger cross-artifact summary endpoint, ticket agent-workbench-vrf) and the lockstep skill-doc change it forces.
3. **Data layer.** Bind the sqlite DB `~/.local/share/artifacts/feedback.db` (configure `DATABASES`), with `managed = False` models + explicit `db_table`/columns mapping the existing schema (artifact_index, comment, thread, ...), so HTTP publish and feedback keep working through the agent-workbench artifact CLI. Do NOT let Django migrations recreate or alter these tables. Document the bd-board mirror behavior (keep it).
