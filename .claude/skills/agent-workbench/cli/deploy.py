...
ARTIFACT_IMAGE = "localhost/artifact-serve:latest"
...
ARTIFACT_HEALTH_URL = "http://127.0.0.1:9099/"
...
def cmd_up(args: argparse.Namespace) -> int:
    ...
    build_image(
        ARTIFACT_IMAGE, str(paths.ARTIFACT_CONTAINER_DIR / "Containerfile"),
    ...
    if unit_active("artifact-serve.service"):
        ...
        if wait_for_http(ARTIFACT_HEALTH_URL):
            print(f"agent-workbench: artifact-serve up at {ARTIFACT_HEALTH_URL}")
        else:
    ...
    if beads_hub_dir.is_dir():
        ...
        print(f"agent-workbench: bd hub not found at {beads_hub_dir} -- "
              "not a service, created on demand by 'agent-workbench bd add'")
    return 0
...
def cmd_down(args: argparse.Namespace) -> int:
    ...
    if quadlet_owned("artifact-serve", artifact_src):
        print("agent-workbench: stopping artifact-serve -- this interrupts the shared review "
